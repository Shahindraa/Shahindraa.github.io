...
function LoadMultipleUsers(userCount){
    ...
    .then(shahi => {
        ...
        for(let i = 0; i < shahi.results.length; i++){
            let fn = '<div>' + shahi.results[i].name.first +'</div>'
            let ln = '<div>' + shahi.results[i].name.last +'</div>'
            let phone = '<div>' + shahi.results[i].phone +'</div>'
            let img = '<img src ="' + shahi.results[i].picture.large +'">'
...
elLoadGender.addEventListener("click",function(){
    ...
    //call loadGender() adn pass gender and number of result
...
function loadGender(gender, userCount){
    ...
    // call, fetch, loop the result and change the innerHTML for allGenderUser
    fetch(url)
    .then(response => response.json())
    .then(data => {
